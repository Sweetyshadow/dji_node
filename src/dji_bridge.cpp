...
public:
  ...
  const tf::Matrix3x3 R_ENU2NED = tf::Matrix3x3(0, 1, 0, 0, 1, 0, 0, 0, -1), 
                      R_FRD2FLU = tf::Matrix3x3(1, 0, 0, 0, -1, 0, 0, 0, -1);

  double control_[4], vel[3], poi[3], offset[3], localPos[3], localVel[3];
  ...
  void commandCallback(const uav_msgs::uav_pose::ConstPtr &msg)
  {
    ...
    offset_[2] = offset[2];
    offsetMutex.unlock();
    control_[0] = msg->position.y - offset_[0];
    control_[1] = msg->position.x - offset_[1];
    control_[2] = -msg->position.z - offset_[2];
    control_[3] = 0;
    // NED_ENU(control_, control_);
    vel_[0] = msg->velocity.x;
    vel_[1] = msg->velocity.y;
    vel_[2] = msg->velocity.z;
    NED_ENU(vel_, vel);
    poi_[0] = msg->POI.y - offset_[0];
    poi_[1] = msg->POI.x - offset_[1];
    ...
    double targetAng = RAD2DEG(atan2f(control_[1] - localPos_[0], control_[0] - localPos_[1]));
    ...
    controlPosYaw.axes.push_back(control_[0] - localPos_[0]);
    controlPosYaw.axes.push_back(control_[1] - localPos_[1]);
    controlPosYaw.axes.push_back(control_[2] - localPos_[2]);
  ...
  bool obtain_control()
  {
    ...
    {
      ROS_ERROR("obtain control_ failed!");
      return false;
    }
